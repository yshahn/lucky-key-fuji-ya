...
function buildPickupSlots() {
  ...
  // NOTE: unlike Hsu's/Pacific Rim, Lucky Key has NO lunch/dinner break —
  // it's continuous service from open to close ("Lunch until 3PM, Dinner
  // from 3PM" back-to-back, per the hours shown on the site). So ASAP is
  // only blocked outside open/close hours, never for a mid-day break.
  const asapBlocked = pastClosing;
  ...
  if (s.allowSchedule !== false) {
    // Single continuous service window (open → close), no midday break.
    const windows = [
      { open: openMins, close: closeMins + 15 },
    ];
    const isMorning  = nowMins < openMins;
    if (pastClosing) {
    ...
    let t;
    if (isMorning) {
      t = (() => { const d = new Date(now); d.setHours(openH, openM, 0, 0); return d; })();
    } else {
      ...
      try {
        ...
        if (_email) {
          ...
          import(_fbUrl).then(async ({ loadPointsFromFirebase, savePointsToFirebase, saveOrderToFirebase }) => {
            ...
            // saveOrderToFirebase gets its OWN retry loop, separate from
            // the points retry below — it used to be called once with an
            // empty catch (after an earlier fix removed it from *inside*
            // the points retry loop, to stop triggering duplicate saves).
            // That left it with no retry at all: a single failure was
            // permanently silent, even though customers still got their
            // email/SMS confirmation, so the order never made it into the
            // admin dashboard and nobody knew. This keeps the "no
            // duplicate saves" fix while actually retrying failures.
