...
async function initServiceStatus() {
  ...
  try {
    ...
    if (cached) { serviceStatus = { reservations: cached.reservations !== false, orders: cached.orders !== false }; applyServiceStatusUI(); }
  } catch(e) {}
...
function applyServiceStatusUI() {
  ...
  [ [reserveBtn, serviceStatus.reservations], [orderBtn, serviceStatus.orders] ].forEach(([btn, isOn]) => {
    if (!btn) return;
...
function isLunchHours() {
  ...
  const start = isWeekend ? 12 * 60 : 11 * 60; // Sat–Sun 12:00PM · Mon–Fri 11:00AM
...
function showLunchClosedPopup() {
  const msg = 'Lunch is served Mon–Fri 11:00 AM – 3:00 PM\nand Sat–Sun 12:00 PM – 3:00 PM.\n\nPlease check back during lunch hours!';
