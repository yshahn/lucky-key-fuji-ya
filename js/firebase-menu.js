// ─────────────────────────────────────────
// Firebase Menu Sync for Lucky Key & Fuji Ya
// (Fully isolated — own Firebase project, separate from Hsu's / Pacific Rim)
// ─────────────────────────────────────────
...
// Firebase project: luckykeyfujiya-3fd7c (isolated — separate from Hsu's / Pacific Rim)
...
// ── Save service ON/OFF status (Reservations / Online Ordering) to Firestore
export async function saveServicesToFirebase(services) {
  try {
    const svcDoc = doc(db, 'config', 'services');
    await setDoc(svcDoc, { data: JSON.stringify(services), updatedAt: Date.now() });
    return true;
  } catch(e) { console.error('Services save error:', e); return false; }
}

// ── Load service ON/OFF status from Firestore
...
// ── Live-listen for service ON/OFF changes (guest-facing banners update instantly)
export function listenServices(callback) {
  ...
  return onSnapshot(svcDoc, snap => {
    if (snap.exists()) {
      try {
        const parsed = JSON.parse(snap.data().data);
        localStorage.setItem('prb_services', JSON.stringify(parsed));
        callback(parsed);
      } catch(e) {}
    }
  });
