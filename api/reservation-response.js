...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: "Lucky Key & Fuji Ya <orders@luckykeyfujiyaga.com>", to: toList, subject, html }),
    });
    return r.ok;
...
module.exports = async function handler(req, res) {
  ...
  if (resDateParsed && !isNaN(resDateParsed.getTime())) {
    ...
    if (resDateParsed.getTime() < todayStart.getTime()) {
      ...
      return res.status(200).send('<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;text-align:center;padding:60px 24px;"><p>This reservation has already passed. If you\'d like to book a new one, please call us at (770) 270-1188 or visit our website.</p></body></html>');
    }
  }
  ...
  try {
    ...
    const fbApp = getApps().length ? getApps()[0] : initializeApp({
      ...
      apiKey: process.env.FIREBASE_API_KEY || 'AIzaSyByZhx5NWLc5u9kJ5x_ZbH6ERGKyFGw7ZE',
    });
    db = getFirestore(fbApp);
    // Firestore rules require an authenticated request to update a reservation
    // (guests never sign in — this endpoint is hit from a plain link). Without
    // signing in here first, every write was silently rejected with
    // PERMISSION_DENIED, which meant the dedup transaction below never actually
    // recorded anything — so every single hit looked like "the first one" and
    // sent a fresh notification every time.
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    <div style="text-align:center;color:#bbb;font-size:12px;border-top:1px solid #eee;padding-top:16px;">
      Lucky Key & Fuji Ya · 4135 Lavista Rd #310, Tucker, GA 30084<br>(770) 270-1188
    </div>
  </div>
</body></html>`,
  });
...
<title>${isConfirmed ? 'See you soon!' : 'Reservation Cancelled'} — Lucky Key & Fuji Ya</title>
<style>
...
<body>
  <div class="card">
    ...
    <p>Questions? Call us at <span class="phone">(770) 270-1188</span></p>
  </div>
</body></html>`;
