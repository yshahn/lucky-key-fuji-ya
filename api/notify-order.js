...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  ...
  if (isReservation) {
    ...
    adminSmsText = [
      ...
      `${pickupLabel}: ${pickup}${curbsideTxt}${deliveryLine}`,
      `──────────────────`,
      adminItemLines,
      `──────────────────`,
    ...
    guestSmsText = [
      ...
      `Hi ${firstName}! ${pickupLabel}: ${pickup}`,
      isCurbside ? `🚗 CURBSIDE${carInfo ? ' · ' + carInfo : ''}` : '',
      isDelivery ? `🏠 ${deliveryAddress || ''}${deliveryFeeAmt ? ' · Delivery fee $' + parseFloat(deliveryFeeAmt || 0).toFixed(2) : ''}` : '',
      guestItemLines,
      guestPtsTxt,
      `💰 Total: $${(total || 0).toFixed(2)}`,
