...
module.exports = async function handler(req, res) {
  ...
  const {
    ...
    pointsDiscount,
    orderType, deliveryFee, deliveryAddress,
    // Notification settings passed from client
    notifEmails, notifPhones,
  } = req.body;

  const RESEND_KEY  = process.env.RESEND_API_KEY;
  const FROM_EMAIL  = process.env.FROM_EMAIL || 'orders@luckykeyfujiya.com';
  ...
  if (req.body.isSaveFailureAlert) {
    ...
    const adminEmails = (notifEmails && notifEmails.length ? notifEmails : ['yshahn@gmail.com', 'ymhahn@gmail.com']);
    ...
    try {
      await sendResendEmail({
        to: adminEmails,
        subject: `⚠️ Order NOT saved to dashboard - Lucky Key & Fuji Ya`,
  ...
  if (req.body.isConfirmationRequest) {
    ...
    const baseUrl = 'https://www.luckykeyfujiya.com';
    ...
    if (guestEmail) {
      await sendResendEmail({
        to: guestEmail,
        subject: `Reservation Confirmation Request — Lucky Key & Fuji Ya`,
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    <div style="text-align:center;color:#bbb;font-size:12px;border-top:1px solid #eee;padding-top:16px;margin-top:24px;">
      Lucky Key & Fuji Ya · 4897 Lavista Rd, Tucker, GA 30084<br>(770) 270-1188
    </div>
  </div>
</body></html>`,
      });
  ...
  const pickupLabel = isDelivery ? '🛵 Delivery' : '📦 Pickup';
  const deliveryFeeAmt = parseFloat(deliveryFee) || 0;
  const deliveryLine = isDelivery ? `\n🏠 ${deliveryAddress || ''}${deliveryFeeAmt ? ' · Delivery fee $' + parseFloat(deliveryFeeAmt || 0).toFixed(2) : ''}` : '';
  ...
  if (isReservation) {
    // ── RESERVATION
    adminSmsText = `🗓️ New Reservation! — Lucky Key & Fuji Ya\n${customer?.name || 'Guest'} · ${pickup}\n👥 ${orderItems[0]?.name || ''}\n📞 ${phone}\n${special ? '📝 ' + special : ''}`;
    ...
    const deliveryTxtPlain = isDelivery ? ` - ${deliveryAddress || 'delivery'}${deliveryFeeAmt ? ' (fee $' + parseFloat(deliveryFeeAmt || 0).toFixed(2) + ')' : ''}` : '';
    adminSmsText = [
      `New order - Lucky Key & Fuji Ya`,
      `${customer?.name || 'Guest'} (${phone})`,
      `${isDelivery ? 'Delivery' : 'Pickup'}: ${pickup}${curbsideTxt}${deliveryTxtPlain}`,
      adminItemLines,
    ...
    const arrivedLink = isCurbside
      ? `\n\n🚗 Curbside Pickup — tap when you arrive:\nhttps://luckykeyfujiya.com?arrived=1&order=${encodeURIComponent(orderIdLocal)}&carModel=${carModelEnc}&carColor=${carColorEnc}`
      : '';
    ...
    guestSmsText = [
      ...
      `Hi ${firstName}! ${isDelivery ? 'Delivery' : 'Pickup'}: ${pickup}${curbsideTxt}${deliveryTxtPlain}`,
      guestItemLines,
      guestPtsTxt,
      `Total: $${(total || 0).toFixed(2)}`,
      specialReq ? `Note: ${specialReq}` : '',
      `Full receipt sent to your email. Questions? (770) 270-1188${arrivedLink}`,
    ].filter(Boolean).join('\n');
    adminEmailSubject = `${isDelivery ? '🛵' : '🥢'} New ${isDelivery ? 'Delivery ' : ''}Order — ${customer?.name || 'Guest'} · $${(total || 0).toFixed(2)} · ${pickup}`;
    guestEmailSubject = `Your Lucky Key & Fuji Ya Order is Confirmed! ${isDelivery ? '🛵' : '🥢'}`;
    ...
    const orderRows = `
      ...
      <tr><td style="padding:5px 0;color:#888;">Phone</td><td style="font-weight:500;">${phone}</td></tr>
      <tr><td style="padding:5px 0;color:#888;">Email</td><td style="font-weight:500;">${guestEmail || '—'}</td></tr>
      ${isDelivery ? `<tr><td style="padding:5px 0;color:#888;">Delivery Address</td><td style="font-weight:500;">${deliveryAddress || '—'}</td></tr>` : ''}`;
    ...
    const totalsHtml = `
      <table style="width:100%;font-size:14px;margin-bottom:16px;">
        ...
        ${isDelivery && deliveryFeeAmt ? `<tr><td style="padding:4px 0;color:#888;">Delivery Fee</td><td style="text-align:right;">$${parseFloat(deliveryFeeAmt || 0).toFixed(2)}</td></tr>` : ''}
        ...
        ${pointsDiscountAmt > 0 ? `<tr><td style="padding:4px 0;color:#27ae60;">✦ Reward Points Discount</td><td style="text-align:right;color:#27ae60;">−$${parseFloat(pointsDiscountAmt || 0).toFixed(2)}</td></tr>` : ''}
    ...
    adminEmailHtml = buildOrderEmailHtml({ firstName, lastName, phone, guestEmail, pickup, pickupLabel, itemsHtml, totalsHtml, orderRows, orderId: orderIdLocal, orderTime, orderCount, total, isAdmin: true, pickupType: req.body.pickupType || 'instore', carModel: req.body.carModel || '', carColor: req.body.carColor || '', specialReq, isDelivery, deliveryAddress });
    guestEmailHtml = buildOrderEmailHtml({ firstName, lastName, phone, guestEmail, pickup, pickupLabel, itemsHtml, totalsHtml, orderRows: '', orderId: orderIdLocal, orderTime, orderCount, total, isAdmin: false, pickupType: req.body.pickupType || 'instore', carModel: req.body.carModel || '', carColor: req.body.carColor || '', specialReq, isDelivery, deliveryAddress });
  }
  ...
  const adminEmailRes = await sendResendEmail({
    ...
    html: adminEmailHtml,
    replyTo: 'orders@luckykeyfujiya.com',
  });
...
function buildOrderEmailHtml({ firstName, lastName, phone, guestEmail, pickup, pickupLabel, itemsHtml, totalsHtml, orderRows, orderId, orderTime, orderCount, total, isAdmin, pickupType, carModel, carColor, specialReq, isDelivery, deliveryAddress }) {
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8"/></head>
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    <div style="background:#f5f2ec;border-radius:10px;padding:16px;margin-bottom:20px;">
      ...
      <p style="margin:4px 0 0;font-size:14px;color:#c8a96e;font-weight:600;">${pickupLabel || '📦 Pickup'}: ${pickup}</p>
      ${isDelivery ? `<p style="margin:4px 0 0;font-size:13px;color:#555;">🏠 ${deliveryAddress || ''}</p>` : ''}
    ...
    ${!isAdmin ? `<div style="background:#f0f9f0;border-left:4px solid #27ae60;padding:14px 16px;border-radius:4px;font-size:13px;color:#555;margin-bottom:20px;">
      Your payment has been processed. ${isDelivery ? 'Your order will be delivered to the address above.' : 'Please pick up your order at the restaurant.'}<br><br>
    ...
    <div style="text-align:center;margin-bottom:20px;">
      ...
      <a href="https://luckykeyfujiya.com?arrived=1&order=${orderId}&carModel=${encodeURIComponent(carModel)}&carColor=${encodeURIComponent(carColor)}"
