...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  ...
  const deliveryLine = isDelivery ? `\n🏠 ${deliveryAddress || ''}${deliveryFeeAmt ? ' · Delivery fee $' + deliveryFeeAmt.toFixed(2) : ''}` : '';
  ...
  if (isReservation) {
    ...
    guestSmsText = [
      ...
      isDelivery ? `🏠 ${deliveryAddress || ''}${deliveryFeeAmt ? ' · Delivery fee $' + deliveryFeeAmt.toFixed(2) : ''}` : '',
      guestItemLines,
      guestPtsTxt,
      `💰 Total: $${(total || 0).toFixed(2)}`,
    ...
    const totalsHtml = `
      <table style="width:100%;font-size:14px;margin-bottom:16px;">
        ...
        ${isDelivery && deliveryFeeAmt ? `<tr><td style="padding:4px 0;color:#888;">Delivery Fee</td><td style="text-align:right;">$${deliveryFeeAmt.toFixed(2)}</td></tr>` : ''}
